...
for paper in papers:
    ...
    if paper['url'] == 'https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4422754':
        papers_html += f"<h3><a href='{paper['url']}'>{paper['title']}</a></h3><h4>Revise and Resubmit (3rd Round - Minor Revision) at the Journal of Financial Economics</h4><p>{', '.join(paper['authors'])}</p><p style='text-align: justify;'>{paper['abstract']}</p>"
